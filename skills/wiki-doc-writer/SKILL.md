...
1. 先判斷文件對象是 Pro SDK 類別、Console/批次程式、MVC/Razor 網頁系統，或多種特徵皆有。
...
5. 若資訊不足但可合理推斷，直接撰寫並在文件中標示「程式碼中未明確顯示」或「需依實際設定確認」。
6. 若關鍵格式會受影響，例如同時有 DockPane、`Program.Main` 與 MVC Controller，先詢問使用者要用哪一種文件格式。
7. 撰寫時優先說明維護者需要知道的責任、資料流、狀態規則、錯誤情境與風險。
...
- 涉及 SQL 條件、路徑、命令、程式碼片段時，使用 fenced code block，並標明語言，例如 `csharp`、`sql`、`text`。
