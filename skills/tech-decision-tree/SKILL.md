...
description: "建立技術選型決策樹與架構建議，且僅使用使用者允許的技術堆疊：C# Console、MVC 全端、Python、ArcGIS Pro SDK for C#（包含 Add-in、Python、Console CoreHost）、ArcGIS Experience、ArcGIS REST API、React、HTML、Razor 與 MSSQL。當需要評估新專案需求、選擇 Web 系統、批次程式、ArcGIS Pro Add-in、Web GIS、REST 整合、Enterprise Geodatabase/SDE 直連、MSSQL GDB schema 權限，或需要說明架構判斷原因時使用。"
---
...
| GIS | `ArcGIS Pro SDK for C#`、`ArcGIS Pro Add-in`、`ArcGIS Pro Python`、`Console CoreHost`、`ArcGIS Experience`、`ArcGIS REST API`、Enterprise Geodatabase / SDE 直連 |
| 前端 | `React`、`HTML`、`Razor` |
| 資料庫 | `MSSQL` |
...
3. 依下列維度分類需求：
   ...
   - `GDB 連線型態`：已發布 REST service、ArcGIS Pro 專案內圖層、Enterprise Geodatabase/SDE 直連、MSSQL 業務表直連。
...
當需求提到 Enterprise Geodatabase、SDE、直連 GDB、dbo/sde schema、`GDB_ITEMS`、FeatureClass 權限、`i..._get_ids`、ArcGIS Server 發布或 MSSQL 裡的空間資料表時，讀取 `references/enterprise-geodatabase-sde.md`。
...
| 需直連 Enterprise Geodatabase/SDE 或 FeatureClass | 優先 `ArcGIS Pro SDK Add-in` 或 `Console CoreHost + MSSQL/SDE`；若只需服務查詢則改用 `ArcGIS REST API` |
...
- 需求資訊不足時，列出使用的假設。
- 視需求補上維護風險，例如 GIS schema 變更、dbo/sde schema 混用、REST service 可用性、排程可觀測性、MSSQL schema 耦合、ArcGIS Pro runtime 限制。
